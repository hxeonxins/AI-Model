...
- **TensorBoard 캡처**: `logs/resnet_20251211-101617` 등 최신 run을 열어 Train vs Val Loss/Acc 그래프 캡처해 첨부.
- **성능 개선 과정 요약**
...
- **최종 모델 성능:** ResNet50 파인튜닝(데이터 40%, 96x96, 3 epoch) Val Acc 0.574 / Loss 0.896.
- **목표 달성 여부:** 현재 95% 목표 미달. CPU 환경/짧은 에포크 제약으로 고성능 달성 불가 → GPU에서 20+ 에포크, 더 큰 입력(128x128)·강한 증강(CutMix/Label Smoothing) 필요.
- **한계/개선:** CIFAR-10 해상도 및 도메인 한계, 학습 시간 부족 → 도메인 데이터로 재학습, 에포크 확장, cosine warmup 등 추가 실험 제안.
